...
import {
  Box, Toolbar, Drawer, AppBar, List, Typography,
  ListItem, ListItemIcon, ListItemText, FormControlLabel, Switch
} from "@mui/material";
...
import RefreshRout from '../../RefreshRout';
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Router>
        ...
        <Routes>
          <Route exact path='/' element={<DrawerContentDisplay />} />
          {RoutesData.map((route, index) => (
            <Route key={index} path={route} element={<RefreshRout isDataAvailable={props.tabData}><DrawerContentDisplay /></RefreshRout>} />
          ))}
        </Routes>
      </Router>
    </Box>
